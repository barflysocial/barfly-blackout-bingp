...
function createSession({ title, origin, cardsPerPlayer=1, cap=40, tipLink='', scheduledAt=null }) {
  const code = makeCode();
  const session = {
    code,
    ...
    cardsPerPlayer: Math.max(1, Math.min(12, Number(cardsPerPlayer)||1)),
...
function resetRound(session, keepPlayers=true) {
  ...
  for (const p of session.players.values()) p.cards = Array.from({length: session.cardsPerPlayer}, makeCard);
...
function createPlayerForSession(session, name, reservation=null) {
  const player = { id:id('p_'), name:cleanName(name, session.players.size), cards:Array.from({length: session.cardsPerPlayer}, makeCard), kicked:false, reservationId: reservation?.id || null };
  updateCalledFlags(session, player);
...
io.on('connection', socket => {
  ...
  socket.on('createSession', async ({ title, cardsPerPlayer, cap, tipLink, date, time, origin }, cb) => {
    const session = createSession({ title, origin, cardsPerPlayer, cap, tipLink, scheduledAt: parseScheduledAt(date, time) });
  ...
  socket.on('setCardsPerPlayer', ({ code, cardsPerPlayer }, cb) => {
    ...
    session.cardsPerPlayer = Math.max(1, Math.min(12, Number(cardsPerPlayer)||1));
    for (const p of session.players.values()) p.cards = Array.from({length: session.cardsPerPlayer}, makeCard);
