...
app.get('/player', (req, res) => {
  ...
  const playerHtml = fs.readFileSync(path.join(__dirname, 'public', 'player.html'), 'utf8')
    .replace(/__OG_URL__/g, `${base}${req.originalUrl || '/player'}`)
    .replace(/__OG_IMAGE__/g, `${base}/assets/title.png`);
  res.send(playerHtml);
});
...
function normalizeSponsor(raw={}) { return { name:String(raw.name||'').trim(), message:String(raw.message||'').trim(), logo:String(raw.logo||'').trim() }; }
function buildSponsors(input={}) {
  const arr = Array.isArray(input.sponsors) ? input.sponsors : [
    { name:input.sponsor1Name ?? input.sponsorName, message:input.sponsor1Message ?? input.sponsorMessage, logo:input.sponsor1Logo ?? input.sponsorLogo },
    { name:input.sponsor2Name, message:input.sponsor2Message, logo:input.sponsor2Logo },
    { name:input.sponsor3Name, message:input.sponsor3Message, logo:input.sponsor3Logo }
  ];
  return [0,1,2].map(i => normalizeSponsor(arr[i] || {}));
}
...
function createSession({ title, origin, cardsPerPlayer=3, cap=40, tipLink='', scheduledAt=null, timezone='', sponsors=[], callIntervalMs=7000 }) {
  const code = makeCode();
  const session = {
    code,
    ...
    sponsors: buildSponsors({ sponsors }),
    reservations: [],
    players: new Map(),
    deck: makeDeck(),
    calledCodes: new Set(),
    ...
    autoTimer:null,
    callIntervalMs: Math.max(3000, Math.min(60000, Number(callIntervalMs)||7000))
  };
...
function publicSession(session) {
  ...
  return {
    ...
    calledCount: session.calledCodes.size,
...
function callNext(session) {
  ...
  if (!next) { session.status='ended'; if (session.autoTimer) clearInterval(session.autoTimer); session.autoTimer=null; session.hostLog.push('All numbers have been called.'); emitAll(session); return; }
...
setInterval(() => {
  ...
  for (const s of sessions.values()) {
    if ((s.status === 'scheduled' || s.status === 'lobby') && s.scheduledAt && s.scheduledAt <= now) {
      s.status = 'running';
      if (!s.currentCall) callNext(s);
      if (!s.autoTimer) {
        s.autoTimer = setInterval(()=>callNext(s), s.callIntervalMs);
      }
      s.hostLog.push(`Scheduled start time reached. Game auto-started with ${(s.callIntervalMs/1000).toFixed(1)}s caller timer.`);
      emitAll(s);
    }
  }
}, 1000);
...
io.on('connection', socket => {
  ...
  socket.on('createSession', async ({ title, cardsPerPlayer, cap, tipLink, date, time, origin, scheduledAtUtc, timezone, sponsors, sponsor1Name, sponsor1Message, sponsor1Logo, sponsor2Name, sponsor2Message, sponsor2Logo, sponsor3Name, sponsor3Message, sponsor3Logo, sponsorName, sponsorMessage, sponsorLogo, callIntervalMs }, cb) => {
    const utc = Number(scheduledAtUtc);
    const scheduledAt = Number.isFinite(utc) && utc > 0 ? utc : parseScheduledAt(date, time);
    const session = createSession({ title, origin, cardsPerPlayer, cap, tipLink, scheduledAt, timezone, callIntervalMs, sponsors: buildSponsors({ sponsors, sponsor1Name, sponsor1Message, sponsor1Logo, sponsor2Name, sponsor2Message, sponsor2Logo, sponsor3Name, sponsor3Message, sponsor3Logo, sponsorName, sponsorMessage, sponsorLogo }) });
  ...
  socket.on('setCallTimer', ({ code, ms }, cb) => {
    const session = sessions.get(String(code||'').trim()); if (!session) return cb?.({ ok:false, error:'Session not found.' });
    session.callIntervalMs = Math.max(3000, Math.min(60000, Number(ms)||7000));
    if (session.autoTimer) { clearInterval(session.autoTimer); session.autoTimer = setInterval(()=>callNext(session), session.callIntervalMs); }
    session.hostLog.push(`Caller timer set to ${(session.callIntervalMs/1000).toFixed(1)} seconds.`);
    emitAll(session); cb?.({ ok:true, callIntervalMs: session.callIntervalMs });
  });
