...
function makeCard(){
  ...
  for(let r=0;r<ROWS;r++){
    grid.push(LETTERS.map((_,c) => {
      if(r===2 && c===2) return { letter:'N', number:'FREE', code:'FREE', marked:true, called:true, free:true };
      return { ...columns[c][r], marked:false, called:false, free:false };
    }));
  }
...
function aggregatePowers(p){
  const out = emptyPowers();
  for(const card of p.cards) for(const k of POWER_POOL) out[k] += card.powers[k] || 0;
  return out;
}
function updateCalledFlags(s,p){ for(const card of p.cards) for(const row of card.grid) for(const cell of row) cell.called = cell.free || s.calledCodes.has(cell.code); }
...
function setPowerPathIfNeeded(card){
  ...
  if(h>0 || v>0) card.powerPath = h >= v ? 'horizontal' : 'vertical';
}
function awardPowersForCard(s,p,card){
  setPowerPathIfNeeded(card);
  if(!card.powerPath) return;
  const validLines = pathLineCount(card);
  const shouldHaveAwards = Math.max(0, validLines - 2); // 3rd, 4th, 5th valid line each earns a random power
...
function playerBest(s,p){
  ...
  for(const card of p.cards){
    const b = { cardId: card.id, lines: completedLines(card), marked: countMarked(card), remaining: remaining(card), blackedOut: isBlackout(card), powerPath: card.powerPath, pathLines: pathLineCount(card) };
  ...
  return best || { cardId:null, lines:0, marked:0, remaining:24, blackedOut:false, powerPath:null, pathLines:0 };
}
function leaderboard(s){
  const needed = WIN_REQUIREMENTS[s.winMode];
  return [...s.players.values()].filter(p=>p.approved).map(p => {
    ...
    return { playerId:p.id, name:p.name, cardId:focus?.cardId, lines:focus?.lines || 0, marked:focus?.marked || 0, remaining:focus?.remaining ?? 24, blackouts, needed, totalRemainingForMode, cards:p.cards.length };
...
function visibleSession(s){ return { code:s.code, title:s.title, status:s.status, winMode:s.winMode, cardsPerPlayer:s.cardsPerPlayer, createdAt:s.createdAt, currentCall:s.currentCall, callId:s.callId, callEndsAt:s.callEndsAt, calledCodes:[...s.calledCodes], winners:s.winners, playerCount:s.players.size, leaderboard:leaderboard(s), qrDataUrl:s.qrDataUrl, currentFreeze:s.currentFreeze } }
function publicPlayer(p){ return { id:p.id, name:p.name, cardCount:p.cards.length, approved:p.approved, powers:aggregatePowers(p), alreadyWon:p.alreadyWon, blackouts:blackoutCount(p) }; }
function playerView(s,p){ updateCalledFlags(s,p); return { session:visibleSession(s), player:{...publicPlayer(p), cards:p.cards, selectedCallId:p.selectedCallId}, myRank:playerBest(s,p), powerLog:s.powerLog.slice(-16) }; }
function hostView(s){
  ...
  return { session:visibleSession(s), players, powerLog:s.powerLog.slice(-30) };
}
...
function stopTimer(s){ if(s?.interval) clearInterval(s.interval); if(s) s.interval=null; }
function nextCall(s){
...
function resetRound(s, keepPlayers=true){
  stopTimer(s); s.status='lobby'; s.deck=makeDeck(); s.calledCodes=new Set(); s.currentCall=null; s.callId=0; s.callEndsAt=0; s.winners=[]; s.currentFreeze=null; s.powerLog=['New round ready. Everyone has 3 fresh random BINGO cards.'];
  if(keepPlayers){ for(const p of s.players.values()){ resetPlayerCards(p); p.approved=true; } } else { s.players.clear(); }
}
function checkAutoWin(s,p){
  ...
  if(blackouts >= needed){
    p.alreadyWon=true;
    const win = { playerId:p.id, name:p.name, at:Date.now(), mode:s.winMode, blackouts };
    s.winners.push(win);
    s.powerLog.push(`BINGO BLACKOUT: ${p.name} won ${s.winMode.toUpperCase()} mode with ${blackouts} completed card${blackouts===1?'':'s'}.`);
    stopTimer(s); s.status='ended'; s.currentCall=null; s.callEndsAt=0;
  }
}
...
function stealTarget(s, thiefId){
  const ranked = leaderboard(s).map(r => s.players.get(r.playerId)).filter(p=>p && p.id !== thiefId && powerTotal(aggregatePowers(p))>0);
...
io.on('connection', socket => {
  ...
  socket.on('setWinMode', ({code, winMode}, cb) => { const s=sessions.get(String(code)); if(!s) return cb?.({ok:false,error:'Session not found'}); if(s.status!=='lobby' && s.status!=='directions') return cb?.({ok:false,error:'Win mode can only be changed before the game starts.'}); s.winMode=normalizeWinMode(winMode); s.powerLog.push(`Host set mode to ${s.winMode.toUpperCase()}.`); emitHost(s); emitPlayers(s); cb?.({ok:true}); });
  socket.on('joinPlayer', ({code, name}, cb) => { const s=sessions.get(String(code)); if(!s) return cb?.({ok:false,error:'Session not found'}); const p={ id:id('p_'), name:String(name||'Player').trim().slice(0,24), cards:Array.from({length:CARDS_PER_PLAYER}, makeCard), approved:false, selectedCallId:null, alreadyWon:false }; s.players.set(p.id,p); socket.join(`session:${s.code}`); socket.join(`player:${p.id}`); cb?.({ok:true,playerId:p.id,state:playerView(s,p)}); emitHost(s); });
  socket.on('resumePlayer', ({code, playerId}, cb) => { const s=sessions.get(String(code)); const p=s?.players.get(String(playerId)); if(!s||!p) return cb?.({ok:false,error:'Player not found'}); socket.join(`session:${s.code}`); socket.join(`player:${p.id}`); cb?.({ok:true,state:playerView(s,p)}); });
  ...
  socket.on('showDirections', ({code}) => { const s=sessions.get(String(code)); if(!s) return; s.status='directions'; stopTimer(s); s.currentCall=null; s.callEndsAt=0; s.currentFreeze=null; s.powerLog.push('Directions are showing before the game begins.'); emitHost(s); emitPlayers(s); });
  socket.on('startGame', ({code}) => { const s=sessions.get(String(code)); if(!s) return; s.status='running'; stopTimer(s); nextCall(s); s.interval=setInterval(()=>nextCall(s), CALL_SECONDS*1000); });
  socket.on('pauseGame', ({code}) => { const s=sessions.get(String(code)); if(!s) return; s.status='paused'; stopTimer(s); emitHost(s); emitPlayers(s); });
  ...
  socket.on('endGame', ({code}) => { const s=sessions.get(String(code)); if(!s) return; stopTimer(s); s.status='ended'; s.currentCall=null; s.callEndsAt=0; s.currentFreeze=null; s.powerLog.push('Host ended the game.'); emitHost(s); emitPlayers(s); });
  ...
  socket.on('markNumber', ({code, playerId, cardId, cellCode}, cb) => {
    ...
    awardAllPowers(s,p); checkAutoWin(s,p); emitHost(s); emitPlayer(s,p); cb?.({ok:true});
  });
  ...
  socket.on('useSwap', ({code, playerId, cardId, cellCode}, cb) => {
    ...
    if(!s||!p||!targetCard) return cb?.({ok:false,error:'Not found'});
    ...
    if(!targetCell || targetCell.marked || targetCell.free) return cb?.({ok:false,error:'Choose an orange unmarked number on this card.'});
    ...
    for(const c of p.cards){
      ...
      const found=c.grid.flat().find(cell=>cell.code===cellCode && cell.marked && !cell.free);
    ...
    emitHost(s); emitPlayer(s,p); cb?.({ok:true});
  });
});
