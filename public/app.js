...
function renderLeaderboard(list=[]){return `<div class="leader">${list.length?list.map((x,i)=>`<div class="leader-row"><div class="rank">${i+1}</div><div><b>${escapeHtml(x.name)}</b><div class="tiny">${cardShort(x.cardId)} • ${x.lines} lines • ${x.marked}/25 marked</div></div><div class="pill">${x.remaining} away</div></div>`).join(''):'<div class="tiny">No approved players yet.</div>'}</div>`}
