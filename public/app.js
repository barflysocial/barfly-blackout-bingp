...
function countdownFromMs(left){ if(left == null) return 'Waiting for host'; if(left <= 0) return 'Starting now'; const total = Math.ceil(left/1000); const h=Math.floor(total/3600); const m=Math.floor((total%3600)/60); const s=total%60; return h ? `${h}:${String(m).padStart(2,'0')}:${String(s).padStart(2,'0')}` : `${m}:${String(s).padStart(2,'0')}`; }
function countdownText(ms){ if(!ms) return 'Waiting for host'; return countdownFromMs(ms - Date.now()); }
