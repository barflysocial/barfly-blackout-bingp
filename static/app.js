...
function winCallParts(n){n=Number(n||0); if(n>=1&&n<=75)return ['W',n]; if(n>=76&&n<=150)return ['I',n-75]; if(n>=151&&n<=225)return ['N',n-150]; return [null,n]}
function label(n,mode='numbers'){if(!n)return '—'; if(mode==='win'){const [l,num]=winCallParts(n); return l?`${l}${num}`:`${n}`} return `${n}`}
function cellLabel(n,mode='numbers'){if(!n)return '—'; if(mode==='win'){const [l,num]=winCallParts(n); return l?`${num}`:`${n}`} return `${n}`}
...
function renderBoard(data){
  ...
  (data.board||[]).forEach((row,ri)=>{
    ...
    html += `<div class="battle-row ${justCleared?'cleared just-cleared':''}">${row.map(n=>{
      ...
      return `<button class="${cls}" data-n="${n}" ${canClick?'':'disabled'}>${cellLabel(n,data.board_mode)}</button>`
...
function renderPowers(data,pid){
  ...
  for(let i=0;i<3;i++){
    const p=powers[i];
    if(p){
      const passive=p.name==='Fire'||p.name==='Recover';
      const note=passive?'<em>Auto</em>':'';
      const disabled=passive?'disabled aria-disabled="true"':'';
      html+=`<button class="power slot filled power-${esc(p.name).toLowerCase()} ${passive?'passive':''}" data-power="${esc(p.name)}" title="${esc(p.name)}${passive?' activates automatically':''}" ${disabled}>${powerIconMarkup(p.name)}<small>${esc(p.name)}${note}</small></button>`;
    }else{
      html+=`<div class="power slot empty"><span>＋</span><small>Empty</small></div>`;
    }
  }
  if(data.fire_active) html+=`<div class="power-status fire-active">🔥 Fire Ready — auto-blocks Freeze</div>`;
  el.innerHTML=html;
  $$('.power.filled[data-power="Freeze"]').forEach(b=>b.onclick=async()=>{const res=await postJSON(`/api/use_power/${pid}`,{name:b.dataset.power}); if(res.message) alert(res.message); if(!res.ok&&res.error) alert(res.error); updatePlayer();});
}
...
function openSharePanel(url,title='Battle Bingo',text='Join Battle Bingo',qrUrl=''){
  ...
  const input=document.getElementById('shareLinkInput'), img=document.getElementById('shareQrImg');
  if(input) input.value=url;
  if(img){ img.style.display='block'; if(qrUrl) img.src=qrUrl; }
  const native=document.getElementById('nativeShareButton');
  if(native) native.onclick=()=>navigator.share?navigator.share({title,text,url}):navigator.clipboard.writeText(url).then(()=>alert('Link copied'));
  const copy=document.getElementById('copyShareButton');
  if(copy) copy.onclick=()=>navigator.clipboard.writeText(url).then(()=>alert('Link copied'));
  m.classList.add('show');
}
function closeSharePanel(){const m=document.getElementById('shareModal'); if(m)m.classList.remove('show')}
...
let tvResultsDismissed=false;
function setTvResultsButton(show){
  const b=document.getElementById('tvShowResultsBtn');
  if(b) b.style.display=show?'inline-flex':'none';
}
function closeTvResults(){
  const m=document.getElementById('winnersModal');
  if(m) m.classList.remove('show');
  tvResultsDismissed=true;
  tvGameOverShown=true;
  setTvResultsButton(true);
}
function showTvResults(){
  const m=document.getElementById('winnersModal');
  if(m) m.classList.add('show');
  tvResultsDismissed=false;
  setTvResultsButton(false);
}
function showTvGameOverIfNeeded(data){
  if(!document.body.classList.contains('tv-app') || !data) return;
  if(data.status!=='ended'){
    tvGameOverShown=false;
    tvResultsDismissed=false;
    setTvResultsButton(false);
    const m=document.getElementById('winnersModal');
    if(m) m.classList.remove('show');
  ...
  const m=document.getElementById('winnersModal'), wr=document.getElementById('winnerResults'), note=document.getElementById('winnerModalNote'), actions=document.getElementById('winnerActions');
  if(wr) wr.innerHTML=renderWinnerCards(data);
  if(note) note.textContent='Final winner results. Close Results to return to the board, then reset from Host.';
  if(actions) actions.innerHTML='<button class="btn ghost inline-btn" onclick="closeTvResults()">Close Results</button>';
  if(tvResultsDismissed){
    setTvResultsButton(true);
    return;
  }
  tvGameOverShown=true;
