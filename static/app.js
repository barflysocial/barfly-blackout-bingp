...
function renderLeaderboard(lb){const el=$('#leaderboard'); if(!el)return; el.innerHTML=(lb||[]).map((p,i)=>`<div class="leader-row rankings-row"><b>${i+1}</b><span>${p.alias}${p.afk?' ⚠️':''}</span><b>${p.lines_left} lines</b><b>${p.points} pts</b></div>`).join('')||'<div class="small">No players yet</div>'}
...
function renderBoard(data){
  ...
  (data.board||[]).forEach((row,ri)=>{
    ...
    html += `<div class="battle-row ${justCleared?'cleared just-cleared':''}">${row.map(n=>{
      ...
      return `<button class="${cls}" data-n="${n}" ${data.mark_mode==='manual'&&called.has(n)&&!marked.has(n)?'':'disabled'}>${label(n,data.board_mode)}</button>`
