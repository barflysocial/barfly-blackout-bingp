...
def game_leaderboard_payload(g):
    ...
    for p in leaderboard(g.id):
        lb.append({'id':p.id,'alias':p.alias,'points':as_int(p.points),'lines_left':lines_left(j(p.cards_json,[]),p),'powers':len(j(p.powers_json,[])),'progress':len(j(p.row_credit_json,{}))%5,'rows':len(j(p.row_credit_json,{})),'afk':False})
    return lb
...
def apply_power(g,p,name):
    ...
    if name=='Recover':
        ...
        called=j(g.called_json,[])
        # Recover is for a missed call, so it cannot be used to mark the current live call.
        missed=[n for n in called if n != g.current_number and n in board_nums and n not in marked]
        if not missed:
            return {'ok':False,'error':'No missed called number available to recover yet.'}
        recovered=missed[-1]
        ok=mark_number(p, recovered, is_auto=True)
        if not ok:
            return {'ok':False,'error':'That missed number could not be recovered.'}
        feed(g.id, f'✚ {p.alias} used Recover to mark {call_label(recovered, g.board_mode)}', 'power')
        check_end(g)
        return {'ok':True,'message':f'Recovered {call_label(recovered, g.board_mode)}.'}

    if name=='Fire':
        p.shield=True
        feed(g.id, f'🔥 {p.alias} activated Fire immunity', 'power')
        return {'ok':True,'message':'Fire is active. The next Freeze against you will be negated.'}
