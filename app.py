...
def start_game_now(g, reason='Game started'):
    ...
    feed(g.id, f'▶️ {reason}. Rules are locked.', 'game')
...
def maybe_update_schedule(g):
    ...
    if g.rsvp_open_at and g.status=='draft' and now>=g.rsvp_open_at:
        g.status='lobby'
        feed(g.id, '✅ RSVP is open. Lobby countdown started.', 'game')
...
def host_action(code):
    ...
    if action=='start':
        ...
        feed(g.id,'⏱️ Start delayed by 5 minutes','game')
    elif action=='lobby_pause':
        g.lobby_paused=True; feed(g.id,'⏸️ Lobby countdown paused','game')
    elif action=='lobby_resume':
        g.lobby_paused=False; feed(g.id,'▶️ Lobby countdown resumed','game')
    elif action=='cancel':
        g.status='cancelled'; feed(g.id,'🚫 Scheduled game cancelled','game')
