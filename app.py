...
POWERS = ['Freeze','Fire','Peek']
LEGACY_POWER_NAMES = {'Number Peek':'Peek', 'Second Chance':'Fire'}
...
def maybe_award_line_powers(p):
    ...
    # Award only 3 powers per game: at 5, 10, and 15 cleared lines. Held inventory max = 3.
...
def award_power(p, threshold):
    ...
    unavailable={normalize_power_name(x.get('name')) for x in held+used if isinstance(x,dict)}
...
def start_game_now(g, reason='Game started'):
    ...
    if not j(g.call_pool_json,[]):
        g.call_pool_json=dump(random.sample(range(1,76),75))
...
def call_next(g):
    ...
    for pp in Player.query.filter_by(game_id=g.id, status='active').all():
        if pp.frozen_number == -1:
            pp.frozen_number = n
            pp.frozen_until_at = datetime.utcnow()+timedelta(seconds=max(1, int((g.countdown or 10)/2)))
...
def defense_check(attacker, target, power_name):
    if power_name == 'Freeze' and target.shield:
        target.shield=False
...
def apply_power(g,p,name):
    ...
    if name=='Peek':
        called=j(g.called_json,[])
        pool=j(g.call_pool_json,[]) or list(range(1,76))
        nextn=next((n for n in pool if n not in called), None)
        feed(g.id, f'👀 {p.alias} used Peek', 'power')
        return {'ok':True,'message':f'Next number: {call_label(nextn, g.board_mode)}'}

    if name=='Fire':
        p.shield=True
        feed(g.id, f'🔥 {p.alias} activated Fire immunity', 'power')
        return {'ok':True,'message':'Fire is active. Your next Freeze attack will be negated.'}
    ...
    if name=='Freeze':
        ...
        return {'ok':True,'message':f'Freeze will block {t.alias} for half of the next call.'}
...
def host_action(code):
    ...
    elif action=='end': g.status='ended'; feed(g.id,'🏁 Game ended','admin'); finalize_stats(g)
