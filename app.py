import os, json, random, string, re, functools
...
def call_label(n):
    if not n: return '—'
    if n <= 15: return f'B-{n}'
    if n <= 30: return f'I-{n}'
    if n <= 45: return f'N-{n}'
    if n <= 60: return f'G-{n}'
    return f'O-{n}'
...
def all_card_nums(cards): return [n for card in cards for row in card for n in row]
def rows_completed(cards, marked):
    ...
    return done
def blackout_count(cards, marked):
    ms=set(marked)
    return sum(1 for card in cards if all(n in ms for row in card for n in row))
def valid_unblocked_row(p, ci, ri):
    return f'{ci}:{ri}' not in j(p.blocked_rows_json, [])
...
def recalc_player(p, newly_marked=None):
    cards=j(p.cards_json,[]); marked=j(p.marked_json,[])
    credited=set(j(p.row_credit_json,{}).keys())
    if newly_marked: p.points += 5
    new_rows=[r for r in rows_completed(cards, marked) if r not in credited]
    for r in new_rows:
        ci,ri=map(int,r.split(':'))
        if not valid_unblocked_row(p,ci,ri):
            continue
        credited.add(r); p.points += 25
        feed(p.game_id, f'🏆 {p.alias} completed a horizontal Bingo row', 'bingo')
        power_cards=set(j(p.power_cards_json,[]))
        card_rows=[x for x in credited if x.startswith(str(ci)+':')]
        if len(card_rows)>=4 and ci not in power_cards:
            award_power(p, ci)
    old_b=p.blackouts; new_b=blackout_count(cards, marked)
    if new_b>old_b:
        times=j(p.blackout_times_json,[])
        for _ in range(new_b-old_b):
            p.points += 50; times.append(now_iso())
            feed(p.game_id, f'🔥 {p.alias} completed Blackout #{len(times)}', 'blackout')
        p.blackouts=new_b; p.blackout_times_json=dump(times)
        if p.blackouts>=3: award_achievement(p,'Blackout Machine')
    p.row_credit_json=dump({r:True for r in credited})
    if len(j(p.powers_json,[]))+len(j(p.used_powers_json,[]))>=3: award_achievement(p,'Triple Threat')

def award_power(p, card_index):
    g=Game.query.get(p.game_id)
    if not g.powers_enabled: return
    ...
    pc=set(j(p.power_cards_json,[])); pc.add(card_index); p.power_cards_json=dump(list(pc))
...
def leaderboard(game_id):
    ...
    def key(p):
        ...
        return (-p.blackouts, latest, -p.points, -rows, -marked)
...
def mark_number(p, n, is_auto=False):
    ...
    if p.frozen_number == n:
        ...
        feed(p.game_id, f'❄️ {p.alias} was frozen and missed {call_label(n)}', 'power')
        return False
    cards=j(p.cards_json,[]); nums=set(all_card_nums(cards))
...
def call_next(g):
    ...
    if prev and g.mark_mode=='manual':
        for p in Player.query.filter_by(game_id=g.id, status='active').all():
            cards=j(p.cards_json,[]); marked=j(p.marked_json,[])
            if prev in all_card_nums(cards) and prev not in marked and p.frozen_number != prev:
                p.afk_misses += 1
                if p.afk_misses==3:
    ...
    if not remaining:
        g.status='ended'; finalize_stats(g); return
    n=remaining[0]
    ...
    for pp in Player.query.filter_by(game_id=g.id, status='active').all():
        if pp.frozen_number == -1:
            pp.frozen_number = n
    feed(g.id, f'📣 Number called: {call_label(n)}', 'call')
    if g.mark_mode=='auto':
        for p in Player.query.filter_by(game_id=g.id, status='active').all():
...
def check_end(g):
    required=g.blackouts_to_win or 3
    winners=Player.query.filter(Player.game_id==g.id, Player.blackouts>=required).count()
    if winners>=g.triple_winners_needed:
        g.status='ended'; feed(g.id, f'🏁 Game over: {winners} player(s) reached {required} blackout(s)', 'game')
        finalize_stats(g)
...
def rsvp(code):
    ...
    if request.method=='POST':
        ...
        p=Player(game_id=g.id, name=name, phone=phone, alias=alias, cards_json=dump(generate_cards()))
...
def api_game(code):
    ...
    for p in leaderboard(g.id):
        lb.append({'id':p.id,'alias':p.alias,'points':p.points,'blackouts':p.blackouts,'powers':len(j(p.powers_json,[])),'progress':len(j(p.row_credit_json,{}))%4,'rows':len(j(p.row_credit_json,{})),'afk':p.afk_misses>=3})
    return jsonify({'code':g.code,'title':g.title,'venue':g.venue,'status':g.status,'current_number':g.current_number,'current_label':call_label(g.current_number),'called':j(g.called_json,[]),'called_labels':[call_label(n) for n in j(g.called_json,[])][-20:],'countdown':game_seconds_remaining(g),'mark_mode':g.mark_mode,'call_mode':g.call_mode,'blackouts_to_win':g.blackouts_to_win,'triple_winners_needed':g.triple_winners_needed,'power_wheel':j(g.power_wheel_json,{}),'leaderboard':lb,'feed':[{'text':f.text,'kind':f.kind} for f in feeds]})

@app.route('/api/player/<int:pid>')
def api_player(pid):
    p=Player.query.get_or_404(pid); g=Game.query.get(p.game_id); maybe_auto_call(g); db.session.commit()
    return jsonify({'id':p.id,'alias':p.alias,'cards':j(p.cards_json,[]),'marked':j(p.marked_json,[]),'clovers':j(p.clovers_json,[]),'row_credit':j(p.row_credit_json,{}),'powers':j(p.powers_json,[]),'used_powers':j(p.used_powers_json,[]),'points':p.points,'blackouts':p.blackouts,'current_number':g.current_number,'current_label':call_label(g.current_number),'status':g.status,'mark_mode':g.mark_mode,'called':j(g.called_json,[]),'called_labels':[call_label(n) for n in j(g.called_json,[])][-20:],'countdown':game_seconds_remaining(g),'power_wheel':j(g.power_wheel_json,{}),'frozen_number':p.frozen_number,'blocked_rows':j(p.blocked_rows_json,[])})
...
def eligible_targets(g,p, require_power=False):
    players=[x for x in leaderboard(g.id) if x.id!=p.id and x.status=='active']
    me_rank={x.id:i for i,x in enumerate(leaderboard(g.id))}.get(p.id,9999)
    eligible=[x for i,x in enumerate(leaderboard(g.id)) if x.id!=p.id and i<=me_rank]
    if require_power: eligible=[x for x in eligible if len(j(x.powers_json,[]))>0]
    return eligible
...
def apply_power(g,p,name):
    if name=='Lucky Spot':
        cards=j(p.cards_json,[]); marked=j(p.marked_json,[]); unmarked=[n for n in all_card_nums(cards) if n not in marked]
    ...
    if name=='Row Block':
        ...
        if not t: return {'ok':True}
        cards=j(t.cards_json,[]); marked=set(j(t.marked_json,[])); candidates=[]
        for ci,card in enumerate(cards):
            for ri,row in enumerate(card):
                if any(n not in marked for n in row): candidates.append(f'{ci}:{ri}')
        if candidates:
    ...
    if name=='Card Shuffle':
        ...
        if not t: return {'ok':True}
        cards=j(t.cards_json,[]); marked=set(j(t.marked_json,[])); unmarked=[n for n in all_card_nums(cards) if n not in marked]; random.shuffle(unmarked); it=iter(unmarked)
        new=[]
        for card in cards:
            nc=[]
            for row in card:
