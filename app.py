...
@app.route('/qr/app.png')
def qr_app_png():
    """QR code for the public Battle Bingo home/RSVP share link."""
    join_url=url_for('rsvp_games', _external=True)
    img=qrcode.make(join_url)
    buf=BytesIO()
    img.save(buf, format='PNG')
    buf.seek(0)
    return send_file(buf, mimetype='image/png', download_name='battle-bingo-rsvp-qr.png')
