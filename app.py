...
def api_player(pid):
    ...
    return jsonify({'id':p.id,'alias':p.alias,'board_mode':g.board_mode,'board':j(p.cards_json,[]),'marked':j(p.marked_json,[]),'clovers':j(p.clovers_json,[]),'row_credit':j(p.row_credit_json,{}),'powers':j(p.powers_json,[]),'used_powers':j(p.used_powers_json,[]),'points':p.points,'full_clear':p.blackouts,'clearout_place':player_clearout_place(p),'lines_left':lines_left(j(p.cards_json,[]),p),'current_number':g.current_number,'current_label':current_label,'status':g.status,'mark_mode':g.mark_mode,'called':j(g.called_json,[]),'called_labels':[call_label(n, g.board_mode) for n in j(g.called_json,[])][-20:],'countdown':game_seconds_remaining(g),'lobby_countdown':lobby_seconds_remaining(g),'lobby_paused':g.lobby_paused,'scheduled_start':format_local_datetime(g.scheduled_start_at, g.schedule_timezone),'power_wheel':j(g.power_wheel_json,{}),'winners':winners,'frozen_number':p.frozen_number,'blocked_rows':j(p.blocked_rows_json,[])})
...
def apply_power(g,p,name):
    if name=='Lucky Spot':
        board=j(p.cards_json,[]); marked=j(p.marked_json,[]); marked_set=set(marked)
        board_nums=set(all_card_nums(board))
        missed=[n for n in j(g.called_json,[]) if n in board_nums and n not in marked_set]
        unmarked=[n for n in all_card_nums(board) if n not in marked_set]
        if not unmarked: return {'ok':False,'error':'No available spaces'}
        # Recovery first: Lucky Spot covers the oldest missed called number before acting as a free mark.
        chosen=missed[0] if missed else random.choice(unmarked)
        marked.append(chosen); clovers=j(p.clovers_json,[]); clovers.append(chosen)
